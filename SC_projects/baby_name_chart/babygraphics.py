...
def get_x_coordinate(width, year_index):
    ...
    """
    x_coord = year_index * (width-2*GRAPH_MARGIN_SIZE)//len(YEARS) + GRAPH_MARGIN_SIZE
    return x_coord
...
def draw_names(canvas, name_data, lookup_names):
    ...
    for i in range(len(lookup_names)):
        color_i = i % len(COLORS)              # decide the line color
        if lookup_names[i] in name_data:
            for j in range(len(YEARS)):
                # Find the coordinate of each point and put on the label.
                if str(YEARS[j]) not in name_data[lookup_names[i]]:        # if the rank is out of 1000:
                ...
                # Draw the line chart.
                if x1 == -1 and y1 == -1:             # the first point of the plot
                    x1 = x2
                    y1 = y2
                else:
